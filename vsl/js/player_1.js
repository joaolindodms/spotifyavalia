...
function vTurbCreatSmartvdsElements() {
    var e, I, i;
    "1.7.9" === vTurbPlayer.version ? document.getElementById(`vid_${vTurbPlayer.id}`) || (window, e = document, I = e.getElementById(`scr_${vTurbSrcId}`), (i = e.createElement("DIV")).id = `vid_${vTurbPlayer.id}`, I.parentElement.insertBefore(i, I)) : (document.getElementById(`vid_${vTurbPlayer.id}`) || function(e, I, i) {
        I = e.getElementById(`scr_${vTurbSrcId}`), (i = e.createElement("DIV")).id = `vid_${vTurbPlayer.id}`, i.style.position = "relative", i.style.width = "100%", i.style.padding = `${vTurbPlayer.video_aspect_ratio}% 0 0`, I.parentElement.insertBefore(i, I)
    }(document), document.getElementById(`thumb_${vTurbPlayer.id}`) || function(e, I, i) {
        I = e.getElementById(`vid_${vTurbPlayer.id}`), (i = e.createElement("IMG")).id = `thumb_${vTurbPlayer.id}`, i.style.top = "0", i.style.left = "0", i.style.width = "100%", i.style.height = "100%", i.style.position = "absolute", i.style.objectFit = "cover", i.src = `https://images.converteai.net/${vTurbPlayer.thumbnail_key}`, I.appendChild(i)
    }(document), document.getElementById(`backdrop_${vTurbPlayer.id}`) || function(e, I, i) {
        I = e.getElementById(`vid_${vTurbPlayer.id}`), (i = e.createElement("DIV")).id = `backdrop_${vTurbPlayer.id}`, i.style.top = "0", i.style.left = "0", i.style.width = "100%", i.style.height = "100%", i.style.position = "absolute", i.style.backdropFilter = "blur(5px)", i.style.webkitBackdropFilter = "blur(5px)", I.appendChild(i)
    }(document))
}

function vTurbLoadSmrtvds() {
    var e, I, i, t;
    e = window, I = document, e.smrtvds || (i = e.smrtvds = function() {
        i.callMethod ? i.callMethod.apply(i, arguments) : i.queue.push(arguments)
    }, e._smrtvds || (e._smrtvds = i), i.push = i, i.loaded = !0, i.version = "1.1", i.queue = [], (t = I.createElement("script")).async = !0, t.src = `https://scripts.converteai.net/lib/js/smartplayer/${vTurbPlayer.version}/smartplayer.min.js`, I.getElementsByTagName("head")[0].appendChild(t)), window.smrtvds(`vid_${vTurbPlayer.id}`, vTurbPlayer.org_id, vTurbPlayer.video_id, vTurbPlayer.options)
}
